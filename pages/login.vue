<template>
  <div class="flex flex-col items-center justify-center px-8" style="min-height: 80vh">
    ...
    <div class="flex gap-2 mb-9">
      <span v-for="t in ['water', 'alcohol', 'move', 'food']" :key="t" class="rounded-full" :class="`grad-${t}`"
        style="width: 34px; height: 6px" />
    </div>

    <button
      class="tap bg-card border border-line text-ink w-full max-w-xs py-4 rounded-3xl font-semibold flex items-center justify-center gap-3 shadow-soft disabled:opacity-60"
      :disabled="loading"
      @click="login">
